...
def tagging_string(source_code, name):
    ...
    for line in source_code.splitlines():
        ...
        if "import " in line:
            new_line = line
            if "team_ufo_swingft" not in line:
        ...
        while idx < len(line):
            ...
            if in_string:
                if char == '\\' and idx + 1 < len(line) and line[idx + 1] == '(':
                    ...
                    match = re.match(rf'\b{re.escape(name)}\b', line[idx:])
                    if match:
    ...
    for path in SWIFT_FILE_PATH:
        ...
        for kind, names in identifier_info.items():
            for name in names:
                ...
                changed_name = mapping_info[original_name]

                # function, variable, case 난독화
                if kind in ["function", "variable", "case"]:
                    pattern_decl = re.compile(rf'(?<![\w"]){re.escape(original_name)}(?![\w"])')
                    source_code = pattern_decl.sub(changed_name, source_code)
                        
                    pattern_member = re.compile(rf'(?<=\.){re.escape(name)}(?![\w"])')
                    source_code = pattern_member.sub(changed_name, source_code)

                # Struct, Protocol, Class, Enum 난독화
                else:
                    pattern = re.compile(rf'(?<![\w"]){re.escape(original_name)}(?![\w"])')
                    source_code = pattern.sub(changed_name, source_code)
